...
class PartitionFilterUtilsSuite extends FunSuite {
  ...
  test("evaluatePredicate") {
    ...
    assert(add1 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 = 0" :: Nil, addFiles))
    assert(add2 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 = 1" :: Nil, addFiles))
    assert(add2 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 > 0" :: Nil, addFiles))
    assert(add1 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 < 1" :: Nil, addFiles))
    assert(add2 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 >= 1" :: Nil, addFiles))
    assert(add1 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 <= 0" :: Nil, addFiles))
    assert(add2 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 <> 0" :: Nil, addFiles))
    assert(add1 :: Nil == evaluatePredicate(schema, "c2" :: Nil, "c2 <> 1" :: Nil, addFiles))
