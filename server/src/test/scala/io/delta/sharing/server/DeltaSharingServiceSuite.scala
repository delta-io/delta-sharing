...
class DeltaSharingServiceSuite extends FunSuite with BeforeAndAfterAll {
  ...
  def readHttpContent(
    ...
    val deltaSharingCapabilities = Seq.newBuilder[String]
    ...
    val result = deltaSharingCapabilities.result()
    if (result.length > 0) {
      connection.setRequestProperty("delta-sharing-capabilities", result.mkString(";"))
    }
