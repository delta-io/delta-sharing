...
class ServerConfigSuite extends FunSuite {
  ...
  def testJsonConfig(serverConfig: ServerConfig): Unit = {
    val tempFile = Files.createTempFile("delta-sharing", ".json").toFile
    try {
      ServerConfig.save(tempFile.getCanonicalPath, serverConfig)
      println(FileUtils.readFileToString(tempFile, UTF_8))
  ...
  def testYamlConfig(serverConfig: ServerConfig): Unit = {
    val tempFile = Files.createTempFile("delta-sharing", ".yaml").toFile
    try {
      ServerConfig.save(tempFile.getCanonicalPath, serverConfig)
      println(FileUtils.readFileToString(tempFile, UTF_8))
      assert(serverConfig == ServerConfig.load(tempFile.getCanonicalPath))
