 ...
 * @param logicalToPhysicalNameMap The mapping from logical name to physical name for columns.
 *                                 Can be empty. Should be used when provided.
 */
case class EvalContext(
    partitionValues: Map[String, String],
    statsValues: Map[String, (String, String)] = Map.empty,
    logicalToPhysicalNameMap: Map[String, String] = Map.empty)
...
trait BaseOp {
  ...
  // Leaf operations may also return a boolean in some special cases.
  ...
  def validateChildren(children: Seq[BaseOp], forV2: Boolean = false): Unit = {
    if (children.size != 1) {
      throw new DeltaSharingUnsupportedOperationException(
        "expected 1 but found " + children.size + " children")
    }
...
trait BinaryOp {
  ...
  def validateChildren(children: Seq[BaseOp], forV2: Boolean = false): Unit = {
    if (children.size != 2) {
      throw new DeltaSharingUnsupportedOperationException(
        "expected 2 but found " + children.size + " children")
    }
...
trait NaryOp {
  ...
  def validateChildren(children: Seq[BaseOp], forV2: Boolean = false): Unit = {
    if (children.size < 2) {
      throw new DeltaSharingUnsupportedOperationException(
        "expected at least 2 but found " + children.size + " children")
    }
    children.map(c => c.validate(forV2))
  }
}
...
case class ColumnOp(val name: String, val valueType: String) extends LeafOp {
  override def validate(forV2: Boolean = false): Unit = {
    if (name == null) {
      throw new DeltaSharingUnsupportedOperationException("Name must be specified")
    }
    if (!OpDataTypes.isSupportedType(valueType, forV2)) {
      throw new DeltaSharingUnsupportedOperationException("Unsupported type")
    }
  }
  ...
  override def evalExpectBoolean(ctx: EvalContext): Boolean = {
    if (valueType != OpDataTypes.BoolType) {
      throw new DeltaSharingUnsupportedOperationException("Unsupported type for boolean")
    }
  ...
  // Map from logical name to physical name if needed.
  //
  ...
  private def resolve(ctx: EvalContext): String = {
    val physicalName = ctx.logicalToPhysicalNameMap.getOrElse(name, name)
    ctx.partitionValues.getOrElse(physicalName, null)
  }
}
...
case class LiteralOp(val value: String, val valueType: String) extends LeafOp {
  ...
  override def validate(forV2: Boolean = false): Unit = {
    if (value == null) {
      throw new DeltaSharingUnsupportedOperationException("Value must be specified")
    }
    if (!OpDataTypes.isSupportedType(valueType, forV2)) {
      throw new DeltaSharingUnsupportedOperationException("Unsupported type")
    }
  ...
  // We support literal evaluation to return boolean if it is of that type.
  override def evalExpectBoolean(ctx: EvalContext): Boolean = {
    if (valueType != OpDataTypes.BoolType) {
      throw new DeltaSharingUnsupportedOperationException("Unsupported type for boolean")
    }
...
object EvalHelper {
  ...
  // Descends down the op tree and returns the names of all the columns being referred
  // to by the op into the map.
  def getColumnNames(op: BaseOp, colNameSet: scala.collection.mutable.Set[String]): Unit = {
    op match {
      case ColumnOp(name, valueType) => colNameSet += name
      case _ =>
    }
    op.getAllChildren().foreach(childOp => getColumnNames(childOp, colNameSet))
  }
  ...
  def validateValue(value: String, valueType: String): Unit = {
    try {
      ...
      case e: Exception =>
        throw new DeltaSharingUnsupportedOperationException("Error validating type")
    }
  }
}
