...
class DeltaSharingRestClientDeltaSuite extends DeltaSharingIntegrationTest {
  ...
  integrationTest("getMetadata") {
    ...
    try {
      ...
      assert(tableMatadata.lines(1) == """{"metaData":{"id":"f8d5c169-3d01-4ca3-ad9e-7dc3355aedb2","format":{"provider":"parquet"},"schemaString":"{\"type\":\"struct\",\"fields\":[{\"name\":\"eventTime\",\"type\":\"timestamp\",\"nullable\":true,\"metadata\":{}},{\"name\":\"date\",\"type\":\"date\",\"nullable\":true,\"metadata\":{}}]}","partitionColumns":["date"],"configuration":{},"createdTime":1619652806049}}""")
    } finally {
      client.close()
    }
  }
  ...
  private def checkCdfTableCdfEnabledTableV1(lines: Seq[String]): Unit = {
    ...
    // VERSION 1: INSERT 3 files
    for (lineNum <- 0 to 2) {
      assert(lines(lineNum).startsWith(addFilePrefix))
  ...
  private def checkCdfTableCdfEnabledTableV2V3Cdc(lines: Seq[String]): Unit = {
    ...
    assert(lines(0).startsWith(cdcFilePrefix))
    assert(lines(0).contains(""""version":2,"timestamp":1651272655000,"expirationTimestamp":"""))
    // VERSION 3: UPDATE 1 row: 1 remove + 1 add
    assert(lines(1).startsWith(cdcFilePrefix))
    assert(lines(1).contains(""""version":3,"timestamp":1651272660000,"expirationTimestamp":"""))
  }
  ...
  private def checkNotnullToNullTableV0Metadata(line: String): Unit = {
    assert(line == """{"metaData":{"id":"1e2201ff-12ad-4c3b-a539-4d34e9e36680","format":{"provider":"parquet"},"schemaString":"{\"type\":\"struct\",\"fields\":[{\"name\":\"name\",\"type\":\"string\",\"nullable\":false,\"metadata\":{}}]}","partitionColumns":[],"configuration":{"delta.enableChangeDataFeed":"true"},"version":0,"createdTime":1668327039667}}""")
  }

  private def checkNotnullToNullTableV2V3Metadata(line: String, version: String): Unit = {
    assert(line == """{"metaData":{"id":"1e2201ff-12ad-4c3b-a539-4d34e9e36680","format":{"provider":"parquet"},"schemaString":"{\"type\":\"struct\",\"fields\":[{\"name\":\"name\",\"type\":\"string\",\"nullable\":true,\"metadata\":{}}]}","partitionColumns":[],"configuration":{"delta.enableChangeDataFeed":"true"},"version":""" + version + ""","createdTime":1668327039667}}""")
  }
  private def checkNotnullToNullTableAddFiles(lines: Seq[String]): Unit = {
    ...
    assert(lines(0).startsWith(addFilePrefix))
    assert(lines(1).startsWith(addFilePrefix))
