...
pytest.mark.skipif(not ENABLE_INTEGRATION, reason=SKIP_MESSAGE)
def test_list_files_in_table_with_embedded_newlines(rest_client: DataSharingRestClient):
    # Testing the updated function with JSON data containing embedded newline characters
    response_content = (
    ...
    lines = iter(response_content.split('\n'))

    rest_client._post_internal = lambda *args, **kwargs: ({
        DataSharingRestClient.DELTA_TABLE_VERSION_HEADER: '2'
    }, lines)

    response = rest_client.list_files_in_table(
        Table(name="table_with_newlines", share="share_with_newlines", schema="default")
    )

    assert response.delta_table_version == 2
    assert response.protocol == Protocol(min_reader_version=1)
    assert response.metadata == Metadata(
        id="12345",
    ...
    assert response.add_files == [
        AddFile(
            ...
            size=1000,
            stats=(
                '{"numRecords": 100, "value": "This is a multiline\nstring with embedded newlines"}'
            ),
        ),
        AddFile(
            url="file2.parquet",
...
@pytest.mark.skipif(not ENABLE_INTEGRATION, reason=SKIP_MESSAGE)
def test_list_files_in_table_partial_json_objects(rest_client: DataSharingRestClient):
    # Testing the handling of partial JSON objects in the response
    response_content = (
        '{"protocol": {"minReaderVersion": 1}}\n'
        ...
        '{"file": {"url": "file4.parquet", "id": "jkl012", "partitionValues": {}, "size": 1700, "stats": "{\"numRecords\": 170}'  # Incomplete JSON object
    )
    lines = iter(response_content.split('\n'))

    rest_client._post_internal = lambda *args, **kwargs: ({
        DataSharingRestClient.DELTA_TABLE_VERSION_HEADER: '3'
    }, lines)

    response = rest_client.list_files_in_table(
        Table(name="table_with_partial_json", share="share_with_partial_json", schema="default")
    )

    assert response.delta_table_version == 3
    assert response.protocol == Protocol(min_reader_version=1)
    assert response.metadata == Metadata(
        id="67890",
        format={"provider": "parquet", "options": {}},
    ...
    assert len(response.add_files) == 1  # Only the valid JSON object should be parsed
    assert response.add_files[0] == AddFile(
        url="file3.parquet",
        id="ghi789",
        partition_values={},
        size=1200,
        stats=(
            '{"numRecords": 120, "value": "Partial\nJSON\nobject"}'
        ),
    )


@pytest.mark.skipif(not ENABLE_INTEGRATION, reason=SKIP_MESSAGE)
def test_list_files_in_table_json_decode_error(rest_client: DataSharingRestClient):
    # Testing handling of JSON decode errors with problematic lines
    response_content = (
        '{"protocol": {"minReaderVersion": 1}}\n'
        ...
        '{"file": {"url": "file6.parquet", "id": "pqr678", "partitionValues": {}, "size": 2500, "stats": "{\"numRecords\": 200, \"value\": \"Another value\"}"}}'
    )
    lines = iter(response_content.split('\n'))

    rest_client._post_internal = lambda *args, **kwargs: ({
        DataSharingRestClient.DELTA_TABLE_VERSION_HEADER: '4'
    }, lines)

    response = rest_client.list_files_in_table(
        Table(name="table_with_json_decode_error", share="share_with_json_decode_error", schema="default")
    )

    assert response.delta_table_version == 4
    assert response.protocol == Protocol(min_reader_version=1)
    assert response.metadata == Metadata(
        id="98765",
        format={"provider": "parquet", "options": {}},
    ...
    assert response.add_files == [
        AddFile(
            ...
            size=2000,
            stats=(
                '{"numRecords": 1074361, "value": "Develop2.1 Pipeline, Veracode "}'
            ),
        ),
        AddFile(
            url="file6.parquet",
            ...
            size=2500,
            stats=(
                '{"numRecords": 200, "value": "Another value"}'
            ),
        )
    ]
