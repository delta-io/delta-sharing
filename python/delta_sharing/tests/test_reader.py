...
def test_table_changes_to_pandas_non_partitioned_delta(tmp_path):
    ...
    pdf1[DeltaSharingReader._commit_timestamp_col_name()] = pd.Series(
        [timestamp1 * 1000] * 3
    ).astype("datetime64[us, UTC]")
    pdf2[DeltaSharingReader._commit_timestamp_col_name()] = pd.Series(
        [timestamp2 * 1000] * 3
    ).astype("datetime64[us, UTC]")
    pdf3[DeltaSharingReader._commit_timestamp_col_name()] = pd.Series(
        [timestamp3 * 1000] * 3
    ).astype("datetime64[us, UTC]")
    pdf4[DeltaSharingReader._commit_timestamp_col_name()] = pd.Series(
        [timestamp4 * 1000] * 3
    ).astype("datetime64[us, UTC]")
