...
* query configuration via `table.scan(...)` and `table.changes(...)`
* Arrow-native reads via `load_as_arrow(...)`, `table.scan(...).to_arrow()`, `table.scan(...).to_record_batches()`, and `table.scan(...).to_record_batch_reader()`
