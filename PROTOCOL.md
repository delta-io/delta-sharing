 ...
 **includeHistoricalProtocol** (type: Boolean, optional): If set to true, return historical [Protocol](#protocol-in-delta-format) actions seen in the delta log so the streaming client can check whether the table is still read compatible (e.g. when a new reader feature is enabled mid-stream). Each inlined historical [Protocol](#protocol-in-delta-format) carries a `version` field identifying the delta log version it corresponds to. This only affects responses with `responseformat=delta`; for `responseformat=parquet` responses the flag is ignored and no additional Protocol actions are emitted. Independent of this flag, on `responseformat=delta` responses the head [Protocol](#protocol-in-delta-format) carries a `version` field stamped with the snapshot version used for the head of the CDF response, mirroring how the head [Metadata](#metadata-in-delta-format) is stamped.</td>
</tr>
</table>
