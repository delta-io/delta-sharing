...
class RemoteDeltaLogSuite extends SparkFunSuite with SharedSparkSession {
  ...
  test("RemoteDeltaLog should not error when the new metadata is a subset of current metadata") {
  ...
  test("RemoteDeltaLog errors when new metadata includes extra fields not in new metadata") {
